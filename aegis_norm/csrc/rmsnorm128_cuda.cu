...
__device__ __forceinline__ float warp_sum128(float value) {
...
__global__ void rmsnorm128_kernel(const scalar_t* x, const scalar_t* weight,
  ...
  local_sum = warp_sum128(local_sum);
  if (lane == 0) warp_sums[warp] = local_sum;
  __syncthreads();

  if (warp == 0) {
    // All 32 lanes participate; only four hold nonzero warp partials.
    float total = lane < 4 ? warp_sums[lane] : 0.0f;
    total = warp_sum128(total);
...
at::Tensor rmsnorm128_launch(const at::Tensor& x, const at::Tensor& weight, float eps) {
  ...
  if (x.scalar_type() == at::kHalf) {
    rmsnorm128_kernel<at::Half><<<static_cast<unsigned int>(rows), 128, 0, stream.stream()>>>(
        x.data_ptr<at::Half>(), weight.data_ptr<at::Half>(), output.data_ptr<at::Half>(), width, eps);
  } else {
    rmsnorm128_kernel<float><<<static_cast<unsigned int>(rows), 128, 0, stream.stream()>>>(
