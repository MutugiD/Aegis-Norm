...
at::Tensor rmsnorm128_launch(const at::Tensor& x, const at::Tensor& weight, float eps);

at::Tensor rmsnorm128_checked(const at::Tensor& x, const at::Tensor& weight, double eps) {
  ...
  return rmsnorm128_launch(x, weight, eps32);
}

TORCH_LIBRARY(aegis_norm_experiments, library) {
  library.def("rms_norm_128(Tensor x, Tensor weight, float eps) -> Tensor");
}

TORCH_LIBRARY_IMPL(aegis_norm_experiments, CUDA, library) {
  library.impl("rms_norm_128", &rmsnorm128_checked);
}

// Reject active gradients explicitly rather than creating a silent wrong backward.
at::Tensor rmsnorm128_autograd(const at::Tensor& x, const at::Tensor& weight, double eps) {
  TORCH_CHECK(!at::GradMode::is_enabled() || (!x.requires_grad() && !weight.requires_grad()),
              "native RMSNorm is inference-only; use reference for active gradients");
  at::AutoDispatchBelowAutograd guard;
  static auto op = c10::Dispatcher::singleton()
      .findSchemaOrThrow("aegis_norm_experiments::rms_norm_128", "")
      .typed<at::Tensor(const at::Tensor&, const at::Tensor&, double)>();
  return op.call(x, weight, eps);
}

TORCH_LIBRARY_IMPL(aegis_norm_experiments, Autograd, library) {
  library.impl("rms_norm_128", &rmsnorm128_autograd);
}
