...
The smoke compiler uses two host jobs, architecture 7.5, optimization level O2 and no global fast-math flag. The standalone smoke now selects a fresh temporary build directory by default and records it; `--reuse-build-cache` opts into caching and labels the result. The notebook passes the fresh build directory to subsequent tests to avoid recompiling. Temporary binaries remain outside the exported evidence archive.
...
[Foundation requirements](../../requirements-foundation.txt) pin direct dependencies. The session's `environment-versions.txt` records resolved versions without exporting private package URLs. It is an inventory, not a portable, hash-locked T4 environment. The received T4 run omits the resolved snapshot; a qualified reproduction lock remains a tracked evidence gap. Keep the wheel index and compiler/driver metadata with that inventory.

The standalone CLI records the current working directory's Git identity; run it from the tested checkout as the notebook does. The notebook installs that checkout before invoking the CLI. Do not attribute an installed package from another commit to the current checkout. The smoke timeout and ordinary interruption kill the Linux worker process group, including compiler children, before recording status. A hard session termination can still leave incomplete evidence. Full interruption/recovery handling follows in F06.
...
The earlier 2.8.0 candidate produced eight known-vulnerability findings in the direct package audit. The 2.14.0 direct audit found none at review time; full resolved-environment audits run separately. C2 replaces the old F01 installation candidate with initial GPU smoke support observed, but without a release compatibility claim. Transformers/model compatibility remains a later F05 qualification, not inherited from this package upgrade.
