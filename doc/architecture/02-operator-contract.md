...
Status: v0.1 contract. F02 implements the reference, inspectable dispatch, explicit native loading and CUDA forward; initial GPU correctness passed; sanitizer and reproduction coverage remain pending. Model adaptation remains specified. [Kernel PRD](../prd/01-kernel-library.md) owns requirements. The separate vector smoke extension does not implement the registered RMSNorm schema.
